...
export function RecycleTable({ onSelectRecord }: RecycleTableProps) {
  ...
  const totalPages = Math.ceil(filteredRecords.length / pagination.pageSize) || 1;
  ...
  const SortIcon = ({ column }: { column: "deletedAt" | "title" | "module" }) => {
                ...
                >
                  ...
                  <td className="px-4 py-3 text-sm">
                    <span className="inline-flex items-center px-2 py-1 rounded text-xs font-medium bg-[var(--background)] text-[var(--foreground)]">
                      {record.currentStorage === "archived" ? "📦 Archived" : "💾 Active"}
                    </span>
                  </td>
      ...
      {filteredRecords.length > 0 && (
        <div className="border-t border-[var(--border)] px-4 py-3 flex items-center justify-between">
          <div className="text-sm text-[var(--secondary)]">
            ...
            {Math.min(pagination.page * pagination.pageSize, filteredRecords.length)} of{" "}
