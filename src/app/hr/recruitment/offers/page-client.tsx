...
export function OffersPageClient() {
  const { loadDemoData, offers, selectedOfferId, selectOffer, checkExpiry } =
    useOffers();
  const { showToast, toasts, removeToast } = useToast();
  const [openNew, setOpenNew] = useState(false);

  useEffect(() => {
    if (offers.length === 0) loadDemoData();
  }, [offers.length, loadDemoData]);
  useEffect(() => {
    const t = setInterval(() => checkExpiry(), 1000 * 60);
      ...
      <div className="dashboard-header-content">
        <div>
          ...
          <p className="dashboard-subtitle">
            Create and manage job offer letters
          </p>
        </div>
        <div className="flex items-center gap-2">
          <button
            onClick={() => setOpenNew(true)}
            className="px-3 py-2 rounded-xl bg-violet-500 text-black"
          >
            + New Offer
          </button>
        </div>
