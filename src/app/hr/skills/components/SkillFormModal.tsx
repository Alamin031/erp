...
import {
  skillFormSchema /*, type SkillFormInput */,
...
export function SkillFormModal({ open, editId, onClose, onSaved }: Props) {
  ...
  const {
    ...
    resolver: zodResolver(skillFormSchema) as Resolver<FormValues>,
    ...
    <AnimatePresence>
      ...
      >
        <motion.div
          ...
          className="w-full max-w-lg rounded-2xl bg-slate-900 border border-slate-800 shadow-2xl"
        >
          ...
          <form onSubmit={handleSubmit(onSubmit as any)} className="p-4 space-y-4">
            ...
            <div className="flex justify-end gap-2 pt-2">
              <button
                type="button"
                onClick={onClose}
                className="px-4 py-2 rounded-xl bg-slate-800 text-slate-200 border border-slate-700 hover:bg-slate-700"
              >
                Cancel
              </button>
              <button
                disabled={isSubmitting}
                className="px-4 py-2 rounded-xl bg-emerald-500 text-black hover:bg-emerald-400"
              >
                {editing ? "Save" : "Create"}
              </button>
            </div>
