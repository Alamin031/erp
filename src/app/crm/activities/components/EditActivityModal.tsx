...
export function EditActivityModal({ isOpen, activity, onClose, onSave, contacts = [], companies = [] }: Props) {
  ...
  useEffect(()=>{ 
    if(isOpen && activity) {
      setForm({ 
        ...
        dateTime: activity.dateTime?.slice(0,16) || '', 
        notes: activity.notes || '', 
        followUp: activity.followUp?.slice(0,16) || '' 
      });
      setError(null);
    }
  }, [isOpen, activity]);
  ...
  const handleSave = ()=>{
    ...
    onSave(activity.id, { 
      ...
      contactName: contacts.find(c=>c.id===form.contactId)?.fullName || activity.contactName, 
      companyName: companies.find(c=>c.id===form.companyId)?.name || activity.companyName 
    });
    onClose();
  };
