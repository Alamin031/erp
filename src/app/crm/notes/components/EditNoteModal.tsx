...
interface Props { isOpen: boolean; note?: Note | null; onClose: ()=>void; onSave?: (id:string, payload:any)=>void; contacts?: any[]; companies?: any[]; deals?: any[]; tags?: Tag[] }

export function EditNoteModal({ isOpen, note, onClose, onSave, contacts = [], companies = [], deals = [], tags = [] }: Props) {
  const [form, setForm] = useState<any>({ title: '', linkedEntityType: 'Contact', linkedEntityId: '', tags: [], description: '', ownerName: '' });
  const [error, setError] = useState<string|null>(null);

  useEffect(()=>{ if(note){ setForm({ title: note.title, linkedEntityType: note.linkedEntityType, linkedEntityId: note.linkedEntityId || '', tags: note.tags || [], description: note.description || '', ownerName: note.ownerName || '' }); } }, [note]);
  if(!isOpen || !note) return null;

  const handleSave = ()=>{
    const res = Schema.safeParse(form);
    if(!res.success){ setError(res.error.issues.map(i=>i.message).join(', ')); return; }
    onSave?.(note.id, { ...form, linkedEntityName: (form.linkedEntityType === 'Contact' ? contacts.find(c=>c.id===form.linkedEntityId)?.fullName : form.linkedEntityType === 'Company' ? companies.find(c=>c.id===form.linkedEntityId)?.name : deals.find(d=>d.id===form.linkedEntityId)?.name) });
    onClose();
  };
